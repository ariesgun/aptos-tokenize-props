...
export const StatsBar: React.FC<{
  ...
  const { market_id, base, quote } = selectedMarket;
  ...
  const { data: iconData } = useQuery({
    queryKey: ["iconData", selectedMarket],
    queryFn: async () => {
  ...
  const { data: priceInfo, isFetching: isFetchingPriceInfo } = useQuery({
    queryKey: ["marketStats", market_id],
    refetchInterval: 10 * 1000,
    queryFn: async () => {
      const response = await fetch(
        `${API_URL}/rpc/market_aggregated_info?market=${market_id}&seconds=86400`,
      );
  ...
  return (
    <>
      ...
      {isModalOpen && isSmallWindow && (
        <div
          className={`fixed inset-0 z-[60] flex h-full w-full items-center justify-center overflow-hidden bg-black text-center font-jost text-2xl font-bold`}
        >
        ...
        <div className="flex overflow-x-clip whitespace-nowrap gap-6">
          ...
          <div className="block md:hidden">
            <p className="font-roboto-mono font-light">
              <span className="inline-block min-w-[4em] text-xl">
      ...
      <div className="flex h-[120px] flex-shrink-0 flex-col justify-center border-b border-neutral-600 px-[29.28px] pr-[30.85px] md:hidden">
          ...
          <div className="flex h-fit items-center">
            ...
            <div className="ml-4 min-w-[160px]">
              ...
              >
                ...
                <ChevronDownIcon className="my-auto ml-5 h-[18px] w-[18px]" />
              </button>
            </div>
          </div>
          <div className="">
            <p className="font-roboto-mono font-light">
              <span className="inline-block min-w-[3.5em]  text-base font-medium">
                {isFetchingPriceInfo && isFirstFetch ? (
                  <Skeleton />
                ) : priceInfo?.last_price != undefined ? (
                  ` $${priceInfo.last_price}`
        ...
        <div className="mt-[5.73px] flex gap-4 pl-[0.07px]">
          ...
          <div className="flex gap-3">
            ...
            <span className="min-w-[3em] font-roboto-mono text-xs font-normal">
              {orderBook.isLoading ? (
                <Skeleton />
              ) : highestBid?.price ? (
                toDecimalPrice({
